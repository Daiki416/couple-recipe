...
- レシピのメイン写真（1 枚）。アップロード元は端末ライブラリ + カメラ撮影の両方
  （`<input type="file" accept="image/*">`・`capture` は付けず両方選べる）。
- クライアントで canvas により長辺 1600px へ縮小し JPEG(品質 0.8) へ再エンコードしてから
  送信する（無料枠節約 + Server Actions ボディ上限回避）。圧縮失敗時は原本を送る。
- 保存先は private バケット `recipe-images`。表示は署名 URL（詳細・一覧・編集のサムネ）。
  一覧は `createSignedUrls` で一括署名し N+1 を避ける。
- 行メタは既存テーブル `recipe_images`（`position=0` の 1 行）を流用する。
- アップロードは Server Action（`createRecipe` / `updateRecipe`）で best-effort 実行。
  失敗してもレシピ保存自体は成功扱いとし、詳細はサーバログのみに記録する。
- Storage アクセスは `ImageStorage` 抽象（`src/lib/storage.supabase.ts` の
  `SupabaseImageStorage`）越しに行い、将来 S3 へ差し替え可能にする。
